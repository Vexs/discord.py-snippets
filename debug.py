...
class Debug:
    ...
    async def scrape_emoji(self, ctx):
        ...
        emoji_session = aiohttp.ClientSession(loop=ctx.bot.loop)

        with ctx.typing():
            for guild in ctx.bot.guilds:
                guildname = guild.name.replace("/", "") # may require more replacing
                for emoji in guild.emojis:
                    async with emoji_session.get(emoji.url) as resp:
                        if resp.status == 200:
                            data = await resp.read()
                            extension = emoji.url.split('.')[-1]
                            emojiname = 'emojis/{guildname}/{emoji.name}.{extension}'.format(
                                guildname=guildname, emoji=emoji, extension=extension)

                            # check if this guild has multiple of the same emoji name, and rename them accordingly
                            multiple_num = 0
                            while emojiname in zip.namelist():
                                multiple_num += 1
                                emojiname = 'emojis/{guildname}/{emoji.name}~{multiple_num}.{extension}'.format(
                                    guildname=guildname, emoji=emoji, multiple_num=multiple_num, extension=extension)
